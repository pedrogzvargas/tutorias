...
from tutorias_itsvc.students.services.estilo_aprendizaje.exceptions import EstiloAprendizajeAlreadyExist


class EstiloAprendizajeCreatorService:
    def __init__(self,
    ...
    def __call__(self,
        ...
        if self.__estilo_aprendizaje_repository.get(student_id=student_id):
            raise EstiloAprendizajeAlreadyExist("Ya existe un registro")
        if not student:
            raise StudentNotExist(f"No existe un estudiante con el id {student_id}")
        return self.__estilo_aprendizaje_repository.create(student_id=student_id,
                                                           p1=p1,
                                                           p2=p2,
                                                           p3=p3,
                                                           p4=p4,
