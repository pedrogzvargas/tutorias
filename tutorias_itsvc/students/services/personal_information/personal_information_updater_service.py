...
class PersonalInformationUpdaterService:
    ...
    def __call__(self,
        ...
        personal_information = self.__personal_information_repository.get(user_id=student.user.id)

        if not personal_information:
            raise PersonalInformationNotExist(f"No se encontró información personal con el id {student_id}")
        ...
        return self.__personal_information_repository.update(
