...
class SubjectGetterController:
    def __init__(self, repository, serializer, response, getter_service=None):
        self.__repository = repository
        self.__serializer = serializer
        self.__response = response
        self.__service = getter_service or StudentSubjectGetterService(self.__repository)
    ...
    def __call__(self, **kwargs):
        try:
            ...
            log.exception(f"Error in SubjectGetterController::__call__, error: {err}")
            response_data = dict(
                success=False,
                message=f"{err}",
            )
