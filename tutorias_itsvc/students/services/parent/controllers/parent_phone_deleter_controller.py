...
from tutorias_itsvc.users.services.person_phone import PersonPhoneDeleterService
...
class ParentPhoneDeleterController:
    def __init__(self, repository, response, service=None):
        self.__repository = repository
        self.__response = response
        self.__service = service or PersonPhoneDeleterService(self.__repository)
    ...
    def __call__(self, student_id, type, phone_id):
        try:
            ...
            log.exception(f"Error in ParentPhoneDeleterController::__call__, error: {err}")
            response_data = dict(
                success=False,
                message=f"{err}",
            )
