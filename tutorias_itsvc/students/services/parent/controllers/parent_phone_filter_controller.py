...
from tutorias_itsvc.users.services.person_phone import PersonPhoneFilterService
...
class ParentPhoneFilterController:
    def __init__(self, repository, serializer, response, getter_service=None):
        self.__repository = repository
        self.__serializer = serializer
        self.__response = response
        self.__service = getter_service or PersonPhoneFilterService(self.__repository)
    ...
    def __call__(self, student_id, type):
        try:
            ...
            phone = self.__service(person_id=parent.id)
            if phone:
            ...
            log.exception(f"Error in ParentPhoneFilterController::__call__, error: {err}")
            response_data = dict(
                success=False,
                message=f"{err}",
            )
