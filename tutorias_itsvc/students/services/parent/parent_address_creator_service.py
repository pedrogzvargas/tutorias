...
from tutorias_itsvc.common.services.address.exceptions import AddressAlreadyExist


class ParentAddressCreatorService:
    def __init__(self,
    ...
    def __call__(self,
        ...
        if parent.address_id:
            raise AddressAlreadyExist(f"Ya existe una dirección para el {type} de estudiante con id {student_id}")

        if not self.__state_repository.get(id=state_id):
            raise StateNotExist(f"No existe un estado con el id {state_id}")

        address = self.__address_repository.create(
            street=street,
        ...
        return self.__parent_repository.update(id=parent.id, address_id=address.id)
