...
from tutorias_itsvc.students.services.scholarship import StudentScholarshipCreatorService
...
class StudentScholarshipCreatorController:
    def __init__(self,
                 request,
                 response,
        ...
        self.__service = service or StudentScholarshipCreatorService(
    ...
    def __call__(self, student_id):
        try:
            ...
            log.exception(f"Error in StudentScholarshipCreatorController::__call__, error: {err}")
            response_data = dict(
                success=False,
                message=f"{err}",
            )
