...
from tutorias_itsvc.students.services.organizacion_estudio import OrganizacionEstudioGetterService
...
class OrganizacionEstudioGetterController:
    def __init__(self,
                 response,
                 serializer,
        ...
        self.__service = service or OrganizacionEstudioGetterService(
            repository=self.__organizacion_estudio_repository,
        )
    ...
    def __call__(self, **kwargs):
        try:
            ...
            log.exception(f"Error in OrganizacionEstudioGetterController::__call__, error: {err}")
            response_data = dict(
                success=False,
                message=f"{err}",
            )
