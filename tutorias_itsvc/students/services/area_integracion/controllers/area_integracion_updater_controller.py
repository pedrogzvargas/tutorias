...
from tutorias_itsvc.students.services.area_integracion import AreaIntegracionUpdaterService
...
class AreaIntegracionUpdaterController:
    def __init__(self,
                 request,
                 response,
        ...
        self.__service = service or AreaIntegracionUpdaterService(
    ...
    def __call__(self, student_id):
        try:
            ...
            log.exception(f"Error in AreaIntegracionUpdaterController::__call__, error: {err}")
            response_data = dict(
                success=False,
                message=f"{err}",
            )
