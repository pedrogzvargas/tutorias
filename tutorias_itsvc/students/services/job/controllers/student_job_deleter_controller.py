...
from tutorias_itsvc.students.services.job import StudentJobDeleterService
...
class StudentJobDeleterController:
    def __init__(self,
                 response,
        ...
        self.__service = service or StudentJobDeleterService(
    ...
    def __call__(self, student_id):
        try:
            ...
            log.exception(f"Error in StudentJobDeleterController::__call__, error: {err}")
            response_data = dict(
                success=False,
                message=f"{err}",
            )
