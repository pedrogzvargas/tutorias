...
from tutorias_itsvc.students.services.job import StudentJobUpdaterService
...
class StudentJobUpdaterController:
    def __init__(self,
                 request,
                 response,
        ...
        self.__service = service or StudentJobUpdaterService(
    ...
    def __call__(self, student_id):
        try:
            ...
            log.exception(f"Error in StudentJobUpdaterController::__call__, error: {err}")
            response_data = dict(
                success=False,
                message=f"{err}",
            )
