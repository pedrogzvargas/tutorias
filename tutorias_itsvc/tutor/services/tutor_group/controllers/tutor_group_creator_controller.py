...
from tutorias_itsvc.tutor.services.tutor_group import TutorGroupCreatorService
...
class TutorGroupCreatorController:
    def __init__(self, request, tutor_group_repository, academic_group_repository, response, service=None):
        ...
        self.__service = service or TutorGroupCreatorService(self.__tutor_group_repository)
    ...
    def __call__(self):
        try:
            ...
            log.exception(f"Error in TutorGroupCreatorController::__call__, error: {err}")
            response_data = dict(
                success=False,
                message=f"{err}",
            )
